...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if (
            hasattr(settings, "INSTAGRAM_APP_ID")
            and settings.INSTAGRAM_APP_ID != ""
            and hasattr(settings, "INSTAGRAM_APP_SECRET")
            ...
            logger.error(
                "Need INSTAGRAM_APP_ID, INSTAGRAM_APP_SECRET, & INSTAGRAM_REDIRECT_URI in settings."
            )
            exit(1)
