...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if hasattr(settings, 'INSTAGRAM_APP_ID') and settings.INSTAGRAM_APP_ID is not None and hasattr(settings, 'INSTAGRAM_APP_SECRET') and settings.INSTAGRAM_APP_SECRET is not None and hasattr(settings, 'INSTAGRAM_REDIRECT_URI') and settings.INSTAGRAM_REDIRECT_URI is not None:
