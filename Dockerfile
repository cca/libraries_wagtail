...
RUN set -ex \
    ...
    # switch to using pipenv instead of venv?
...
ENV DJANGO_SETTINGS_MODULE=libraries.settings.production DJANGO_DEBUG=off

# @TODO switch to gunicorn instead of all this uWSGI stuff
# Tell uWSGI where to find your wsgi file:
ENV UWSGI_WSGI_FILE=libraries/wsgi.py
...
RUN mkdir -p /code/libraries/media/images && chown -R 1000:2000 /code/libraries/media

# mark the destination for images as a volume
VOLUME ["/code/libraries/media/images/"]
